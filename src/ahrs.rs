...
/// ジャイロバイアスの係数（対角成分）
const BETA_DIAG: Vector3<f64> = [0.0032, 0.0032, 0.0032];
...
impl ExUdFilter {
    ...
    fn calc_jacobian_h(&mut self) {
        ...
        let r1 = self.x[7];
        let r2 = self.x[8];
        let r3 = self.x[9] + STANDARD_GRAVITY;
        ...
        // dh1/dx3
        self.H[0][7] = 1.0 - 2.0 * (q3*q3 + q2*q2);
        self.H[0][8] = 2.0 * (q0*q3 + q1*q2);
        self.H[0][9] = 2.0 * (q1*q3 - q0*q2);
        self.H[1][7] = 2.0 * (q1*q2 - q0*q3);
        self.H[1][8] = 1.0 - 2.0 * (q1*q1 + q3*q3);
        self.H[1][9] = 2.0 * (q0*q1 + q2*q3);
        self.H[2][7] = 2.0 * (q0*q2 + q1*q3);
        self.H[2][8] = 2.0 * (q2*q3 - q0*q1);
        self.H[2][9] = 1.0 - 2.0 * (q2*q2 + q1*q1);
...
#[allow(dead_code)]
/// MatrixNxNを整形してプロット
pub fn plot_nn(m: &MatrixNxN<f64>) {
    println!("MatrixNxN = [");
    for i in 0..SYS_N {
        print!("    [");
        for j in 0..SYS_N {
...
pub fn calc_h(x: VectorN<f64>) -> VectorP<f64> {
    ...
    let h1 = quat::frame_rotation(q, [x[7], x[8], x[9] + STANDARD_GRAVITY]);
